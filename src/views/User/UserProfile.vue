<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <!-- Profile Form -->
      <div class="bg-white rounded-lg p-6 shadow-sm border border-silver-gray lg:col-span-2">
        <h2 class="text-lg font-semibold text-highlight-text mb-4">Personal Information</h2>
        ...
        <form @submit.prevent="updateProfile" class="space-y-4">
          ...
          <div class="border-t pt-6 mt-6">
            <h3 class="text-lg font-semibold text-highlight-text mb-4">Wallet Addresses</h3>
            ...
            <div class="space-y-4">
              <div v-for="currency in availableCurrencies" :key="currency">
                <label :for="`wallet-${currency}`" class="form-label">{{ currency }} Wallet Address</label>
                <input
                  :id="`wallet-${currency}`"
      ...
      <div class="space-y-6">
        ...
        <div class="bg-white rounded-lg p-6 shadow-sm border border-silver-gray">
          <h3 class="text-lg font-semibold text-highlight-text mb-4">Account Summary</h3>
          <div class="space-y-3">
            <div class="flex justify-between items-center">
              <span class="text-gray-600">Member Since:</span>
              <span class="font-medium text-sm">{{ formatDate(authStore.user?.createdAt) }}</span>
            </div>
            <div class="flex justify-between items-center">
            ...
            <div class="flex justify-between items-center">
              ...
              <span
                :class="[
                  ...
                  authStore.user?.kyc ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                ]"
              >
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="font-medium text-sm capitalize">{{ authStore.user?.role }}</span>
            </div>
          </div>
        </div>
        ...
        <!-- Trading Stats -->
        <div class="bg-white rounded-lg p-6 shadow-sm border border-silver-gray">
          <h3 class="text-lg font-semibold text-highlight-text mb-4">Trading Statistics</h3>
          <div class="space-y-3">
            <div class="flex justify-between items-center">
              <span class="text-gray-600">Total Balance:</span>
              <span class="font-semibold">{{ formatCurrency(authStore.user?.balance || 0) }}</span>
            </div>
            <div class="flex justify-between items-center">
            ...
            <div class="flex justify-between items-center">
              ...
              <span class="font-semibold">{{ authStore.user?.stats?.trades || 0 }}</span>
            </div>
          </div>
        </div>
        ...
        <!-- Security Actions -->
        <div class="bg-white rounded-lg p-6 shadow-sm border border-silver-gray">
          <h3 class="text-lg font-semibold text-highlight-text mb-4">Security</h3>
          <div class="space-y-3">
            <router-link
              to="/user/kyc"
              class="flex items-center p-3 border border-silver-gray rounded-lg hover:bg-cool-light transition-colors duration-200"
            >
              <font-awesome-icon :icon="['fas', 'id-card']" class="h-5 w-5 text-primary-blue mr-3" />
            ...
            >
              <font-awesome-icon :icon="['fas', 'trash']" class="h-5 w-5 mr-3" />
    ...
    >
      <div class="bg-white rounded-lg max-w-md w-full p-6">
        <div class="flex items-center mb-4">
          <font-awesome-icon :icon="['fas', 'exclamation-triangle']" class="h-6 w-6 text-red-600 mr-3" />
          <h3 class="text-lg font-semibold text-highlight-text">Delete Account</h3>
        </div>
        
        <p class="text-gray-600 mb-4">
          Are you sure you want to delete your account? This action cannot be undone. All your data, including investments and transaction history, will be permanently deleted.
        </p>
